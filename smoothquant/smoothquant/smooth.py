...
def smooth_ln_fcs(ln, fcs, act_scales, alpha=0.5):
    ...
    for fc in fcs:
        print(ln.weight.numel())
        print(fc.in_features)
        print(act_scales.numel())
...
def smooth_ln_fcs_llama_like(ln, fcs, act_scales, alpha=0.5):
    if not isinstance(fcs, list):
        fcs = [fcs]
    assert isinstance(ln, (LlamaRMSNorm, MistralRMSNorm, MixtralRMSNorm, MambaRMSNorm))
    for fc in fcs:
        assert isinstance(fc, nn.Linear)
        assert ln.weight.numel() == fc.in_features == act_scales.numel()
    device, dtype = fcs[0].weight.device, fcs[0].weight.dtype
...
def smooth_lm(model, scales, alpha=0.5):
    for name, module in model.named_modules():
        if isinstance(module, OPTDecoderLayer):
            ...
            attn_ln = module.norm  # attention forward norm
            mixer = module.mixer
            qkv = [
                mixer.in_proj,
                mixer.x_proj,
            ...
            qkv_input_scales = scales[name + ".mixer.in_proj"]
            smooth_ln_fcs_llama_like(attn_ln, qkv, qkv_input_scales, alpha)