...
def quantize_mamba(
    ...
    for name, m in model.named_modules():
        if isinstance(m, MambaMixer):
            # Her we simulate quantizing BMM inputs by quantizing the output of q_proj, k_proj, v_proj
            m.in_proj = W8A8Linear.from_float(
                m.in_proj,
                weight_quant=weight_quant,
                act_quant=act_quant,
                quantize_output=quantize_bmm_input,
            )
            m.x_proj = W8A8Linear.from_float(
                m.x_proj,
                weight_quant=weight_quant,
                act_quant=act_quant,
                quantize_output=quantize_bmm_input,
            )
            m.dt_proj = W8A8Linear.from_float(
                m.dt_proj,
                weight_quant=weight_quant,
                act_quant=act_quant,
                quantize_output=quantize_bmm_input,
...
def quantize_model(
    ...
    from transformers.models.mamba.modeling_mamba import MambaPretrainedModel
    ...
    elif isinstance(model, MambaPretrainedModel):
        return quantize_mamba(
            model,
            weight_quant=weight_quant,
            act_quant=act_quant,
            quantize_bmm_input=quantize_bmm_input,
