...
class QJambaAttention(nn.Module):
    """
    ...
    def __init__(self, original_layer):
        ...
        if original_layer.layer_idx is None:
            logger.warning_once(
        ...
        self.num_key_value_heads = original_layer.num_key_value_heads
        ...
        self.q_proj = QLinearLayer(originalLayer=original_layer.q_proj)
        self.k_proj = QLinearLayer(originalLayer=original_layer.k_proj)
        self.v_proj = QLinearLayer(originalLayer=original_layer.v_proj)
        self.o_proj = QLinearLayer(originalLayer=original_layer.o_proj)

        act_quant = True
        if act_quant:
            self.qact_query = QAct(tensor_name="query_quant")
            self.qact_key = QAct(tensor_name="key_quant")
            self.qact_value = QAct(tensor_name="value_quant")
            self.qact_attn = QAct(tensor_name="attn_quant")
            self.qact_proj = QAct(tensor_name="proj_quant")
        else:
            self.qact_query = lambda x: x
            self.qact_key = lambda x: x
            self.qact_value = lambda x: x
            self.qact_attn = lambda x: x
            self.qact_proj = lambda x: x

        # self.output_had = HadamardTransform()

        self.query_smooth = SmoothModule(weight_to_smooth="q_proj")
        self.key_smooth = SmoothModule(weight_to_smooth="k_proj")
        self.value_smooth = SmoothModule(weight_to_smooth="v_proj")
        self.output_smooth = SmoothModule(weight_to_smooth="o_proj")
...
class QJambaFlashAttention2(QJambaAttention):
    """
...
class QJambaSdpaAttention(QJambaAttention):
    """
    ...
    def forward(
        ...
        bsz, q_len, _ = hidden_states.size()
        query_states = self.query_smooth(hidden_states)
        query_states = self.qact_query(query_states)

        key_states = self.key_smooth(hidden_states)
        key_states = self.qact_key(key_states)

        value_states = self.value_smooth(hidden_states)
        value_states = self.qact_value(value_states)
        ...
        attn_output = self.output_smooth(attn_output)
        # attn_output = self.output_had(attn_output)
        attn_output = self.qact_attn(attn_output)
        attn_output = self.o_proj(attn_output)

        attn_output = self.qact_proj(attn_output)

        return attn_output, None, past_key_value


JAMBA_ATTENTION_CLASSES = {
    "eager": QJambaAttention,
    "flash_attention_2": QJambaFlashAttention2,
    "sdpa": QJambaSdpaAttention,
}
...
class QJambaMambaMixer(nn.Module):
    ...
    def __init__(self, original_layer):
        ...
        self.layer_idx = original_layer.layer_idx
        self.hidden_size = original_layer.hidden_size
        self.ssm_state_size = original_layer.ssm_state_size
        self.conv_kernel_size = original_layer.conv_kernel_size
        self.intermediate_size = original_layer.intermediate_size
        self.time_step_rank = original_layer.time_step_rank
        self.use_conv_bias = original_layer.use_conv_bias
        self.use_bias = original_layer.use_bias
        self.conv1d = original_layer.conv1d
        self.activation = original_layer.activation
        self.act = original_layer.act
        self.apply_inner_layernorms = original_layer.apply_inner_layernorms

        self.use_fast_kernels = original_layer.use_fast_kernels
        ...
        self.A_log = original_layer.A_log
        self.D = original_layer.D
        self.out_proj = original_layer.out_proj

        if self.apply_inner_layernorms:
            self.dt_layernorm = original_layer.dt_layernorm
            self.B_layernorm = original_layer.B_layernorm
            self.C_layernorm = original_layer.C_layernorm
        else:
        ...
        self.conv1d = QConv1D(originalLayer = original_layer.conv1d)
        self.in_proj = QLinearLayer(originalLayer=original_layer.in_proj)
        ...
        smooth=True
        if smooth:
            self.mamba_in_smooth = SmoothModule(weight_to_smooth="in_proj")
            self.u_smooth = SmoothModule(weight_to_smooth="x_proj")
            self.dt_smooth = SmoothModule(weight_to_smooth="dt_proj")
            self.ssm_out_smooth = SmoothModule(weight_to_smooth="out_proj")
        else:
            self.mamba_in_smooth = lambda x: x
            self.u_smooth = lambda x: x 
    ...
    def cuda_kernels_forward(self, hidden_states: torch.Tensor, cache_params: MambaCacheParams = None):
        assert torch.isfinite(hidden_states).all(), "Input tensor hidden_states contains NaNs."
        ...
        if (
            ...
            contextualized_states = mamba_inner_fn(
                projected_states,
                self.conv1d.fake_quant_weight,
                self.conv1d.bias if self.use_conv_bias else None,
                self.x_proj.fake_quant_weight,
                self.dt_proj.fake_quant_weight,
                self.out_proj.fake_quant_weight,
            ...
            hidden_states = self.conv_in_quant(hidden_states)
            conv_weights = self.conv1d.weight.view(self.conv1d.fake_quant_weight.size(0), self.conv1d.fake_quant_weight.size(2))
            ...
            hs = self.u_smooth(hidden_states.transpose(1, 2))
            hs = self.u_quant(hs)
            ssm_parameters = self.x_proj(hs)
            ...
            discrete_time_step = self.dt_smooth(time_step)
            discrete_time_step = self.dt_quant(discrete_time_step)
            discrete_time_step = self.dt_proj(discrete_time_step).transpose(1, 2)
            ...
            time_proj_bias = self.z_quant(time_proj_bias.unsqueeze(1).unsqueeze(1)).flatten()
            ...
            scan_outputs = self.ssm_out_smooth(scan_outputs)
            scan_outputs = self.ssm_out_had(scan_outputs)
            scan_outputs = self.ssm_out_quant(scan_outputs)
            
            # scan_outputs = self.ssm_out_had(scan_outputs, transpose=True)
    ...
    def slow_forward(self, input_states, cache_params: MambaCacheParams = None):
        ...
        if cache_params is not None:
            ...
            if cache_params.seqlen_offset > 0:
                ...
                hidden_states = torch.sum(conv_state * self.conv1d.sim_quant_weight[:, 0, :], dim=-1)
        ...
        # discrete_A = self.A_quant()
        # discrete_B = self.B_quant
        # 3.c perform the recurrence y ← SSM(A, B, C)(x)
        scan_outputs = []
        ...
        scan_output = self.ssm_out_had(scan_output)
        scan_output = self.ssm_out_quant(scan_output)
        scan_output = self.ssm_out_had(scan_output, transpose=True)
        if cache_params is not None:
            cache_params.ssm_states[self.layer_idx].copy_(ssm_state)
        # 4. Final linear projection
...
class QJambaMLP(nn.Module):
    def __init__(self, original_layer, act_quant=True):
        super().__init__()
        self.ffn_dim = original_layer.ffn_dim
        self.hidden_dim = original_layer.hidden_dim
        ...
        self.gate_proj = QLinearLayer(originalLayer=original_layer.gate_proj)
        self.down_proj = QLinearLayer(originalLayer=original_layer.down_proj)
        self.up_proj = QLinearLayer(originalLayer=original_layer.up_proj)

        self.act_fn = original_layer.act_fn

        smooth=False
        if smooth:
            self.up_smooth = SmoothModule("up_proj")
            self.gate_smooth = SmoothModule("gate_proj")
            self.down_smooth = SmoothModule("down_proj")
        if act_quant:
            self.qact_up = QAct(tensor_name="qact_up")
            self.qact_gate = QAct(tensor_name="qact_gate")
            self.qact_down = QAct(tensor_name="qact_down")
            self.qact_out = QAct(tensor_name="qact_out")
        else:
            self.qact_up = lambda x: x
            self.qact_gate = lambda x: x
            self.qact_down = lambda x: x
            self.qact_out = lambda x: x
    ...
    def forward(self, x):
        ...
        g = self.gate_smooth(x)
        g = self.qact_gate(g)
        g = self.gate_proj(g)

        v = self.act_fn(g) * u

        v = self.down_smooth(v)
        v = self.qact_down(v)
        out = self.down_proj(v)

        out = self.qact_out(out)
        return out


# Adapted from transformers.models.mixtral.modeling_mixtral.MixtralSparseMoeBlock with Mistral->Jamba
class QJambaSparseMoeBlock(nn.Module):
    """
    ...
    def __init__(self, original_layer, act_quant=True):
        ...
        self.num_experts = original_layer.num_experts
        self.top_k = original_layer.top_k

        if self.num_experts > 1:
            # expert routing
            self.router = QLinearLayer(original_layer.router)
        else:
            self.router = None

        # self.router_smooth = SmoothModule(weight_to_smooth="router")
        if act_quant:
            self.qact_router = QAct(tensor_name="qact_router")
        else:
            self.qact_router = lambda x: x

        self.experts = nn.ModuleList([QJambaMLP(original_layer=original_layer.experts[i], act_quant=act_quant) for i in range(self.num_experts)])
    ...
    def forward(self, hidden_states: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        hidden_states = self.qact_router(hidden_states)
        hidden_states = hidden_states.view(-1, hidden_dim)

        # router_logits: (batch * sequence_length, n_experts)
        # hidden_states = self.router_smooth(hidden_states)
...
class QJambaAttentionDecoderLayer(nn.Module):
    def __init__(self, original_layer):
        super().__init__()

        self.self_attn = original_layer.self_attn
        self.moe = original_layer.moe
        self.input_layernorm = original_layer.input_layernorm
        self.pre_moe_layernorm = original_layer.pre_moe_layernorm
...
class QJambaMambaDecoderLayer(nn.Module):
    def __init__(self, original_layer):
        super().__init__()
        self.mamba = original_layer.mamba
        self.moe = original_layer.moe
        self.input_layernorm = original_layer.input_layernorm
        self.pre_moe_layernorm = original_layer.pre_moe_layernorm
...
class QJambaPreTrainedModel(PreTrainedModel):
    config_class = JambaConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["JambaAttentionDecoderLayer", "QJambaMambaDecoderLayer"]
...
class QJambaModel(QJambaPreTrainedModel):
    """
    ...
    def __init__(self, originalModel):
        super().__init__(originalModel.config)
        config = originalModel.config
        ...
        for i in range(config.num_hidden_layers):
            ...
            if is_attn:
                decoder_layers.append(QJambaAttentionDecoderLayer(original_layer=originalModel.layers[i]))
            else:
                decoder_layers.append(QJambaMambaDecoderLayer(original_layer=originalModel.layers[i]))

        if not any(isinstance(layer, QJambaAttentionDecoderLayer) for layer in decoder_layers):
            raise ValueError("At least one layer in the decoder must be an attention layer")
        self._attn_layer_index = [isinstance(layer, QJambaAttentionDecoderLayer) for layer in decoder_layers].index(
            True
        )

        if not any(isinstance(layer, QJambaMambaDecoderLayer) for layer in decoder_layers):
            raise ValueError("At least one layer in the decoder must be a Mamba layer")
        self._mamba_layer_index = [isinstance(layer, QJambaMambaDecoderLayer) for layer in decoder_layers].index(True)
...
class QJambaForCausalLM(QJambaPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, originalModel):
        super().__init__(originalModel.config)
        self.model = QJambaModel(originalModel.model)
        self.vocab_size = originalModel.vocab_size
        self.lm_head = originalModel.lm_head
        self.router_aux_loss_coef = originalModel.router_aux_loss_coef
        self.num_experts = originalModel.num_experts
        self.num_experts_per_tok = originalModel.num_experts_per_tok
        # Initialize weights and apply final processing
        self.post_init()
